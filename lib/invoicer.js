    ...
    _.each(matches, function (match) {
      if(match) {
        ...
        if(_.isObject(input[key])) {
          data = data.replace(regexp,util.format(input.currencyFormat, parseFloat(input[key].value).toFixed(2)));
        } else {
          if(input[key]) {
  ...
  if (items) {
   items.forEach(function (element) {
        out += row
          ...
          .replace(/PRICE/g, util.format(currencyFormat, element.price ? parseFloat(element.price).toFixed(2) : 0))
          .replace(/TAX/g, element.tax ? element.tax * 100 + '%' : '0%')
          .replace(/TOTAL/g, util.format(currencyFormat, element.total ? parseFloat(element.total).toFixed(2) : 0));
      });
    }
