  ...
  if (items) {
    items.forEach(function (element) {
      out += row
          ...
          .replace(/PRICE/g, element.price ? '€ ' + Number(parseFloat(element.price)).toFixed(2) : '€' + '0.00')
          .replace(/TAX/g, element.tax ? element.tax * 100 + '%' : '0%')
          .replace(/TOTAL/g, element.total ? '€ ' + Number(parseFloat(element.total)).toFixed(2) : '€' + '0.00');
    });
  }
