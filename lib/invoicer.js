  ...
  if (items) {
   items.forEach(function (element) {
        out += row
          ...
          .replace(/PRICE/g, util.format(currencyFormat, element.price ? element.price.toFixed(2) : 0))
          .replace(/TAX/g, element.tax ? element.tax * 100 + '%' : '0%')
          .replace(/TOTAL/g, util.format(currencyFormat, element.total ? element.total.toFixed(2) : 0));
      });
    }
